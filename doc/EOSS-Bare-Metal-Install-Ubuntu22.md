...
Now run the `setupnewhome.bash` script to create the /eosstracker directory and clone the `eosstracker` GitHub repo to it.  This will also create the file, /etc/rc.local.
```
cd /tmp/eosstracker
...
Once the system comes back online, check that the following directory structure exists.  If there are problems, check that the /etc/rc.local file is executable and is free of any syntax errors.
```
