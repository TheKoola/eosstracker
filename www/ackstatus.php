 ...
 */
    ...
    if (isset($_GET["thetime"])) {
        $get_thetime = strtoupper(check_string($_GET["thetime"], 30));
    }
    else
        $get_thetime = "";
    ...
    // Check the statusmsg HTML GET variable
    if (isset($_GET["statusmsg"])) {
        $get_statusmsg = check_string($_GET["statusmsg"], 64);
    }
    else
        $get_statusmsg = "";

    ## if any of the GET parameters are not supplied, then exit...
    if ($get_thetime == 0 || $get_callsign == "" || $get_statusmsg == "") {
    ...
    $query = "insert into statusacknowledgements values ($1, $2, $3);";
    $result = pg_query_params($link, $query, array(
        sql_escape_string($get_thetime), 
        sql_escape_string($get_callsign),
        sql_escape_string($get_statusmsg)
    ));
    if (!$result) {
        printf ("{\"result\" : 0, \"error\": %s}", json_encode(sql_last_error()));
        sql_close($link);
