...
function createCoordsHTML(geojson) {
    ...
    let html = "Coords: " + (URL ? "<a target=\"_blank\" href=\"" + URL + "\">" : "") + "<span id=\"" + id + "-coords\">" + lat + ", " + lon + "</span>" + (URL ? "</a>" : "") 
...
function createSpeedHTML(geojson) {
    ...
    let html = (typeof(geojson.properties.altitude) == "undefined" ? "" : (geojson.properties.altitude != 0 && geojson.properties.altitude != "" ? "Alt: " + (geojson.properties.altitude * 10 / 10).toLocaleString() + "ft" : ""));
    html += (speedValid ? " &nbsp; " + (geojson.properties.speed_mph * 10 / 10).toFixed(0) + " MPH &nbsp; @ " + (geojson.properties.bearing * 10 / 10).toFixed(0) + "&deg;" : "");
...
function createHeardfromHTML(geojson) {
    ...
    let heardfrom = (typeof(geojson.properties.frequency) == "undefined" ? "" : (geojson.properties.frequency != "" ? "<font class=\"pathstyle\">Heard on: " + (geojson.properties.frequency / 1000000)  + 
    ...
    if (typeof(geojson.properties.source) != "undefined") {
        if (geojson.properties.source != "direwolf" && geojson.properties.source != "ka9q-radio") 
            heardfrom = "<font class=\"pathstyle\">Heard from TCPIP</font>";
    }
...
function breadcrumbPopup(geojson) {
    ...
    // Get the various info bits about this breadcrumb
    let speed = createSpeedHTML(geojson);
    let coords = createCoordsHTML(geojson);
    let vrate = Math.round(geojson.properties.point.z_velocity * 60);

    // The HTML string for the popup content.
    let html = "<div><font style=\"float:left; margin:0; padding:0;\"><strong>" + fid + "</strong></font>";
    ...
    (vrate ? "<br>Vert Rate: " + vrate.toLocaleString() + " ft/min" : "") + 
...
function stationPopup(geojson) {
    ...
    // The followme link
    html = html + "<font style=\"float:left; margin:0; padding:0;\"><a target=\"_blank\" href=\"testmap.php" +
    ...
    // Get the various info bits about this station 
    let heardfrom = createHeardfromHTML(geojson);
    let speed = createSpeedHTML(geojson);
    let coords = createCoordsHTML(geojson);
...
***********/
async function getMapStyle(url) {
    ...
    // Examine the javascript returned
    if (d) {
        ...
        statusbox.show("Unable to load map");
