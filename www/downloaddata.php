...
include_once $documentroot . '/common/functions.php';
    ...
    if ($formerror == false) {
        if ($datatype == "GPS") {
        ...
        else if (preg_match('/^FLIGHT_/', $datatype)) {
        ...
        else if ($datatype == "ALLRFDATA") {
            $query = "select
                  p.tm::date as thedate,
                  date_trunc('second', p.tm)::time without time zone as thetime,
                  p.source as packet_source,
                  p.channel as direwolf_channel,
                  round(p.frequency / 1000000, 6) as frequency_mhz,
                  p.callsign,
                  p.symbol,
                  p.ptype as packet_type,
                  pt.description as packet_type_description,
                  p.speed_mph,
                  p.bearing as bearing_degrees,
                  p.altitude as altitude_ft,
                  p.comment,
                  round(cast(ST_Y(p.location2d) as numeric), 6) as lat,
                  round(cast(ST_X(p.location2d) as numeric), 6) as lon,
                  case
                      when p.raw similar to '% [-]{0,1}[0-9]{1,6}T[-]{0,1}[0-9]{1,6}P%' then
                          round(32 + 1.8 * cast(substring(substring(substring(p.raw from ' [-]{0,1}[0-9]{1,6}T[-]{0,1}[0-9]{1,6}P') from ' [-]{0,1}[0-9]{1,6}T') from ' [-]{0,1}[0-9]{1,6}') as decimal) / 10.0, 2)
                      else
                  ...
                  from
                  packets p
                  left outer join packettypes pt on (p.ptype = pt.ptype)
                  left outer join symbols s on (p.symbol = s.symbol)

                  where
                  (p.source like 'direwolf%' or p.source like 'ka9q-radio%')
                  and p.tm >= $1
                  and p.tm <= $2
            ...
            if ($numrows > 0) {
                header("Content-Type: text/csv");
                header('Content-Disposition: attachement; filename="allrfpackets.csv"');
                $f = fopen("php://output", "w");
                fputcsv($f, array("datetime", "packet_source", "direwolf_channel", "frequency_mhz", "callsign", "aprs_symbol", "packet_type", "packet_type_description", "speed_mph", "bearing_degrees", "altitude_ft", "lat", "lon", "comment", "temperature_f", "pressure_atm", "md5_hash", "raw_packet"));
                while ($row = sql_fetch_array($result)) {
                    fputcsv($f, array((string)($row["thedate"] . " " . $row["thetime"]), 
                        (string)$row['packet_source'],
                        $row['direwolf_channel'],
                        $row['frequency_mhz'],
                        (string)$row['callsign'],
                        (string)$row['symbol'],
                        (string)$row['packet_type'],
                        (string)$row['packet_type_description'],
                        round($row["speed_mph"], 2), 
                        round($row["bearing"], 0), 
                        round($row["altitude_ft"], 0), 
                        round($row["lat"], 6), 
