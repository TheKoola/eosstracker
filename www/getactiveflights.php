...
' order by thetime asc, a.callsign ;'; 
    ...
    if ($numrows > 0) {
        ...
        foreach ($positioninfo_r as $callsign => $ray) {
            ...
            json_encode($ray[1]), 
            json_encode($ray[4]), 
            json_encode("Flight: <strong>" . $get_flightid. "</strong><br>" . $ray[5] . "<br>Speed: " . $ray[6] . " mph<br>Heading: " . $ray[7] . " deg<br>Vert Rate: " . number_format($ray[8]) . " ft/min"), 
            //json_encode("bla bla"),
            json_encode($callsign), 
            json_encode($ray[6]), 
            json_encode($ray[7]), 
            ...
            foreach ($ray as $k => $elem) {
                ...
                if ($elem[2] != $peak_altitude && $i < $numpoints) {
                    ...
                    printf ("{ \"type\" : \"Feature\", \"properties\" : { \"id\" : %s, \"time\" : %s, \"packet_time\" : %s, \"flightid\" : %s, \"callsign\" : %s, \"symbol\" : \"/J\", \"altitude\" : %s, \"comment\" : %s, \"objecttype\" : \"balloonmarker\", \"ascending\" : %s, \"speed\" : %s, \"heading\" : %s, \"verticalrate\" : %s, \"iconsize\" : %s },", 
                        ...
                        json_encode($callsign), 
                        json_encode($elem[2]),  
                        json_encode("Flight: <strong>" . $get_flightid . "</strong><br>Speed: " . $elem[3] . " mph<br>Heading: " . $elem[4] . " deg<br>Vert Rate: " . number_format($elem[6]) . " ft/min"), 
