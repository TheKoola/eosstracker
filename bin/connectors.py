            ...
            try:
                ...
                if isready[0]:
                    ...
                    if not data:
                        ...
                        while newline in bigbuffer:

                            # split out indivdual lines from the bigbuffer
                            line, bigbuffer = bigbuffer.split(newline, 1)
...
class AprsisStream(PacketStream):
        ...
        if p.text:
            ...
            if len(s) > 1:
                ...
                if "information" in p.properties:
                    ...
                    information_part.encode(encoding = 'utf-8', errors = 'ignore')
    ...
    def igatingFilter(self, p: Packet)->Packet:
        ...
        if properties:
            ...
            if "information" in properties:
                ...
                #self.logger.info(f"{self.server.nickname} igatingFilter. info length: {len(info)}")

                if len(info) > 0:
                    packettype = info[0]
                    #self.logger.info(f"{self.server.nickname} igatingFilter.  {packettype=}, {chr(packettype)}")
    ...
    def getAprsFilter(self)->str:
        ...
        if self.taptype == 'aprs':
            ...
            # for cwop, we really just want a radius filter.  So we average the position of this station with where all the landing locations are at.
