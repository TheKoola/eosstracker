        ...
        if properties:
            if "digipeaters" in properties:
                ...
                for d in properties["digipeaters"]:
                    ...
                    if 'TCPIP' in dupper or 'TCPXX' in dupper or 'RFONLY' in dupper or 'NOGATE' in dupper:
                        self.logger.debug(f"{self.server.nickname} igatingFilter.  Not igating RF only packet: {p}")
                        return None
            else:
                # digipeaters weren't broken out....this shouldn't happen, right?
                self.logger.debug(f"{self.server.nickname} igatingFilter.  Digipeater list wasn't available: {p}")
                return None
            ...
            if p.frequency == 145825000:
                if "digipeaters" in properties:
                    ...
                    self.logger.debug(f"{self.server.nickname} igatingFilter.  Satgate.  Digipeater list wasn't available: {p}")
                    return None
            ...
            if "decode_timestamp" in properties:
                ...
                if epoch - int(properties["decode_timestamp"]) > self.igating_time_limit:
                    self.logger.debug(f"{self.server.nickname} igatingFilter.  Packet age decode_timestamp={int(properties['decode_timestamp'])} is too old to igate: {p}")
                    return None
