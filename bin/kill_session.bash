...
if [ $? -eq 0 ]; then
    ps -ef | egrep "direwolf|aprsc|gpsd|killsession|kill_session|habtracker-daemon" | grep -v grep >> ${LOGFILE}
    ...
    if [ $num_procs -gt 0 ]; then
        echo "Killing lead habtracker-daemon process again..." >> $LOGFILE
        kill $leadproc >> ${LOGFILE}
    fi
