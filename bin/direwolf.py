...
class Direwolf:
    """
    This will look to start direwolf with a fairly basic, slimmed down configuration.  Such that
    direwolf is only looking to:
    ...
    * the KISS port is using the default 8001 port.
        ...
        + The kiss port is also use to have direwolf transmit arbitary packets (the infocmd.py process)
    """
            ...
            if "gpsdata" in g: 
                ...
                if "mode"  in position:
                    ...
                    if mode == 3:
                        ...
                        nofix = False
                        gpsposition["altitude"] = position["altitude"]
                        gpsposition["latitude"] = position["lat"]
                        gpsposition["longitude"] = position["lon"]
    ...
    def createDirewolfConfig(self)->bool:
        ...
        if self.configuration["mobilestation"] == "false":
            ...
            self.logger.info(f"Station location: {gpsposition}")
            ...
            with open(self.config_file, "w") as f:
                ...
                if self.configuration["mobilestation"] == "true":
                    ...
                    if gpsposition["isvalid"] == True:
                        ...
                        f.write("PBEACON sendto=" + str(channel) + " delay=0:30 every=11:00 altitude=" + str(float(gpsposition["altitude"]) * .3048) + " lat=" + str(gpsposition["latitude"]) + " long=" + str(gpsposition["longitude"]) + " via=" + str(eoss) + "WIDE2-1  symbol=" + str(self.configuration["symbol"]) + overlay + " comment=\"" + str(self.configuration["comment"] + "\"\n"))
